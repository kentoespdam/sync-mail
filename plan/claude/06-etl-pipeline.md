...
PENTING: Selalu utamakan mencari referensi terbaru di internet. Jika perlu gunakan `context 7` untuk pencarian data *best practice*, dokumentasi library Python mutakhir, dan pola optimasi agar kode yang dihasilkan *up-to-date*, aman, dan berkinerja tinggi. Khusus untuk task ini, prioritaskan riset: praktik `cursor.executemany()` MariaDB & kalkulasi `max_allowed_packet`, pola generator + context manager Python 3.14, idiom keyset pagination single-column dan komposit, serta perbandingan strategi cast ENUM→VARCHAR (apakah `str()` cukup atau perlu lookup tabel mapping).
